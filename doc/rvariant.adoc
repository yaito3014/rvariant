...
* Let `Us` denote the corresponding template parameter pack for each flexibility-related functions. Let `VT~_i_~` denote `{recursive_wrapper}<T~_i_~, A>` (for any type `A`) if such a specialization occurs anywhere in `Ts\...`; otherwise, let `VT~_i_~` denote `T~_i_~`. Define `WT~_j_~` analogously for the subject with types `Us\...`, where `WT~_j_~` refers to the _j_^th^ type in `Us`. The [.underline]#_corresponding alternative_# for `rvariant` is the first type for which `std::is_same_v<{unwrap_recursive_t}<VT~_i_~>, {unwrap_recursive_t}<WT~_j_~>>` is `true`.
...
** -- [.underline]#`std::is_constructible_v<VT~_i_~, WT~_j_~ const&>` is `true` for all _j_.#
--
+
...
+
[none]
** -- The exception specification is equivalent to the logical `AND` of [.underline]#`std::is_nothrow_constructible_v<VT~_i_~, WT~_j_~ const&>` for all _j_.#
...
** -- `std::is_constructible_v<VT~_i_~, [.underline]#WT~_j_~&&#>` is `true` for all _j_.
--
+
*_Effects:_* `_GET_<w.index()>(w)` is replaced with `_GET_<w.index()>([.underline]#std::move(w)#)`.
+
...
+
[none]
** -- The exception specification is equivalent to the logical `AND` of `std::is_nothrow_constructible_v<VT~_i_~, [.underline]#WT~_j_~&&#>` for all _j_.
...
** -- [.underline]#`{subset_of}<rvariant<Us\...>, rvariant>` is `true`, and#
** -- [.underline]#`std::disjunction_v<std::is_same<rvariant<Us\...>, {unwrap_recursive_t}<Ts>>\...>` is `false`, and#
** -- [.underline]#`std::is_constructible_v<VT~_i_~, WT~_j_~ const&> && std::is_assignable_v<VT~_i_~&, WT~_j_~ const&>` is `true` for all _j_.#
--
+
...
** -- Otherwise, if [.underline]#`std::is_same_v<{unwrap_recursive_t}<T~_i_~>, {unwrap_recursive_t}<WT~_j_~>>` is `true`#, assigns [.underline]#`_GET_<__j__>(rhs)`# to the value contained in `*this`. [.underline]#(_Note:_ the left hand side is `T~_i_~`, _not_ `VT~_i_~`. This ensures that the existing storage is reused even for `rvariant` with duplicate corresponding alternatives; i.e. `index()` is unchanged.)#
** -- Otherwise, if either `std::is_nothrow_constructible_v<VT~_i_~, WT~_j_~ const&>` is `true` or `std::is_nothrow_move_constructible_v<VT~_i_~>` is `false`, equivalent to `emplace[.underline]##<VT~_i_~>##(_GET_<__j__>(rhs))`.
...
*_Returns:_* `*this`.
+
*_Remarks:_* The exception specification is equivalent to the logical `AND` of [.underline]#`std::is_nothrow_constructible_v<VT~_i_~, WT~_j_~ const&> && std::is_nothrow_assignable_v<VT~_i_~&, WT~_j_~ const&>` for all _j_.#
...
** -- [.underline]#`{subset_of}<rvariant<Us\...>, rvariant>` is `true`, and#
** -- [.underline]#`std::disjunction_v<std::is_same<rvariant<Us\...>, {unwrap_recursive_t}<Ts>>\...>` is `false`, and#
** -- [.underline]#`std::is_constructible_v<VT~_i_~, WT~_j_~&&> && std::is_assignable_v<VT~_i_~&, WT~_j_~&&>` is `true` for all _j_.#
--
+
*_Effects:_* Let _j_ be `rhs.index()`.
+
...
** -- Otherwise, if [.underline]#`std::is_same_v<{unwrap_recursive_t}<T~_i_~>, {unwrap_recursive_t}<WT~_j_~>>` is `true`#, assigns [.underline]#`_GET_<__j__>(std::move(rhs))`# to the value contained in `*this`. [.underline]#(_Note:_ the left hand side is `T~_i_~`, _not_ `VT~_i_~`. This ensures that the existing storage is reused even for `rvariant` with duplicate corresponding alternatives; i.e. `index()` is unchanged.)#
** -- Otherwise, equivalent to `emplace[.underline]##<VT~_i_~>##(_GET_<__j__>(std::move(rhs)))`.
--
+
*_Returns:_* `*this`.
+
*_Remarks:_* The exception specification is equivalent to the logical `AND` of [.underline]#`std::is_nothrow_constructible_v<VT~_i_~, WT~_j_~&&> && std::is_nothrow_assignable_v<VT~_i_~&, WT~_j_~&&>` for all _j_.#
