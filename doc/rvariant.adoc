...
Let `charT` denote `_select-char-t_<Fmts\...>`.
+
*_Returns:_* `{variant-format-string}<charT, Ts\...>{std::forward<Fmts>(fmts)\...}`, with `Ts` being the template parameter pack of the cv-unqualified non-reference type for `Variant`.
