...
{
    struct BinaryExpr;
    using Expr = temp_ns::rvariant<A, B, C, temp_ns::<<rvariant.recursive,recursive_wrapper>><BinaryExpr>>;
...
* [.underline]#Let `T` denote an arbitrary type. If a user provides both `T` and `<<rvariant.recursive,recursive_wrapper>><T>` for the template parameter of `rvariant`, the program is ill-formed.#
...
* [.candidate]#4)# @saki7 thinks this is almost equivalent to the standard counterpart, except for the points below. TODO: need implementation to confirm this.
+
*_Postconditions:_* `holds_alternative[.underline]##<<<rvariant.recursive.helper,unwrap_recursive_t>><T~_j_~>>##(*this)` is `true`.

* [.candidate]#5)# [.underline]#Let `WT` denote `<<rvariant.recursive,recursive_wrapper>><T>` if there is any occurrence of `<<rvariant.recursive,recursive_wrapper>><T>` in `Ts\...`; otherwise, let `WT` denote `T`.#
+
*_Mandates:_* [.underline]#`T` is not a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
+
*_Constraints:_*
+
--
[none]
** -- There is exactly one occurrence of `T` in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>\...`# and
...
* [.candidate]#6)# [.underline]#Let `WT` denote `<<rvariant.recursive,recursive_wrapper>><T>` if there is any occurrence of `<<rvariant.recursive,recursive_wrapper>><T>` in `Ts\...`; otherwise, let `WT` denote `T`.#
+
*_Mandates:_* [.underline]#`T` is not a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
+
*_Constraints:_*
...
* [.candidate]#3)# @saki7 thinks this is almost equivalent to the standard counterpart, except for the points below. TODO: need implementation to confirm this.
+
*_Postconditions:_* `holds_alternative[.underline]##<<<rvariant.recursive.helper,unwrap_recursive_t>><T~_j_~>>##(*this)` is `true`, with `T~_j_~` selected by the imaginary function overload resolution described above.
...
* [.candidate]#1)# *_Mandates:_* [.underline]#`T` is not a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
+
*_Constraints:_* `std::is_constructible_v<T, Args\...>` is `true`, and `T` occurs exactly once in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Effects:_* Equivalent to: +
pass:quotes[&nbsp;&nbsp;]`return emplace<__I__>(std::forward<Args>(args)\...);` +
where `_I_` is the zero-based index of `T` in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.

* [.candidate]#2)# *_Mandates:_* [.underline]#`T` is not a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
+
*_Constraints:_* `std::is_constructible_v<T, std::initializer_list<U>&, Args\...>` is `true`, and `T` occurs exactly once in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Effects:_* Equivalent to: +
...
* [.candidate]#2)# The member typedef `type` denotes [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><T~_I_~>`#.
+
...
[.candidates]
* [.candidate]#{empty}# *_Mandates:_* The type `T` occurs exactly once in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Returns:_* `true` if `v.index()` is equal to the zero-based index of `T` in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
...
+
*_Returns:_* [.underline]#`<<rvariant.recursive.helper,_UNWRAP_RECURSIVE_>>(o)`, where `o` denotes a reference to the object stored in the `rvariant`#.
...
* [.candidate]#{empty}# *_Mandates:_* `I < sizeof\...(Ts)`.
+
*_Effects:_* If `v.index()` is `I`, returns [.underline]#`<<rvariant.recursive.helper,_UNWRAP_RECURSIVE_>>(o)`, where `o` denotes a reference to the object stored in the `rvariant`#. Otherwise, throws an exception of type {bad-variant-access}.
...
* [.candidate]#{empty}# *_Mandates:_* The type `T` occurs exactly once in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Effects:_* [.underline]#Let `VT` denote the type of the value held by `v`. If `<<rvariant.recursive.helper,unwrap_recursive_t>><VT>` is the same type as `T`#, returns [.underline]#`<<rvariant.recursive.helper,_UNWRAP_RECURSIVE_>>(o)`, where `o` denotes a reference to the object stored in the `rvariant`#. Otherwise, throws an exception of type {bad-variant-access}.
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
...
+
*_Returns:_* A pointer to the value [.underline]#denoted by `<<rvariant.recursive.helper,_UNWRAP_RECURSIVE_>>(o)`, where `o` denotes a reference to the object stored in the `rvariant`#, if `v != nullptr` and `v\->index() == I`. Otherwise, returns `nullptr`.
...
[.candidates]
* [.candidate]#1-2)# *_Mandates:_* The type `T` occurs exactly once in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Effects:_* Equivalent to: `return get_if<__i__>(v);` with _i_ being the zero-based index of `T` in [.underline]#`<<rvariant.recursive.helper,unwrap_recursive_t>><Ts>`#.
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of `<<rvariant.recursive,recursive_wrapper>>`.#
...
[.candidates]
* [.candidate]#{empty}# *_Effects:_* Denotes `*o`, if cv-unqualified non-reference type for `T` is a specialization of `<<rvariant.recursive,recursive_wrapper>>`. Otherwise, denotes `o`.
...
[WARNING]
`compact_alternative` does not unwrap `<<rvariant.recursive,recursive_wrapper>>`. This is intentional, because doing so could lead to instantiating incomplete type on undesired timings. You may apply `<<rvariant.recursive.helper,unwrap_recursive_t>>` manually.
