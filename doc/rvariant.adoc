...
// basic construction
xref:#rvariant.constructors[AB ab{123}];
...
xref:#flexible-construction[AB ab = BA{}];   // unordered construction
xref:#flexible-construction[ABC abc = AB{}]; // subset construction
...
public:
    ...
    template<class... Us>
      constexpr rvariant(rvariant<Us...> const& w) noexcept(/* see below */);
    template<class... Us>
...
** This constructor is defined as deleted unless `xref:#rvariant.expo.subset[subset-like]<rvariant<Ts\...>, rvariant<Us\...>>` and `std::is_copy_constructible_v<U~_j_~>` is `true` for all _j_, where `U~_j_~` be the _j_^th^ type in `Us\...`.
** If `std::is_trivially_copy_constructible_v<U~_j_~>` is `true` for all _j_, this constructor is trivial.
...
** This constructor is defined as deleted unless `xref:#rvariant.expo.subset[subset-like]<rvariant<Ts\...>, rvariant<Us\...>>`.
...
** If `std::is_trivially_move_constructible_v<U~_j_~>` is `true` for all _j_, this constructor is trivial.
** In case of duplicate alternative types, the corresponding index in `rvariant` shall be determined by the first occurrence of the same type in `Ts\...`.
--
...
This section demonstrates internal features used in the implementation. The documentation is written _as if_ the content were proposed for the (imaginary) C++ standard.


[[rvariant.expo.subset]]
=== Subset Manipulation
