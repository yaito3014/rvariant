...
* In [rvariant], `_GET_` denotes a set of exposition-only function templates (xref:#rvariant.get[[rvariant.get\]]).
...
#include <variant>     // for compatibility with {bad-variant-access}
...
// xref:#rvariant.helper[[rvariant.helper\]], pass:quotes[`rvariant`] helper classes
...
// xref:#rvariant.recursive.helper[[rvariant.recursive.helper\]], pass:quotes[`recursive_wrapper`] helper classes
...
// xref:#rvariant.pack[[rvariant.pack\]], pack manipulation and deduping
...
+
*_Throws:_* {bad-variant-access} if `*this` holds an alternative that is not contained in `Us`; otherwise, throws any exception thrown by copy-constructing any type in `Us`.
+
*_Remarks:_*
+
[none]
...
+
*_Throws:_* {bad-variant-access} if `*this` holds an alternative that is not contained in `Us`; otherwise, throws any exception thrown by move-constructing any type in `Us`.
+
*_Remarks:_*
+
[none]
...
* [.candidate]#2)# The member typedef `type` denotes pass:quotes,macros[`[.underline\]#xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;T~_I_~&gt;#`].
+
...
[.candidates]
* [.candidate]#{empty}# *_Mandates:_* The type `T` occurs exactly once in pass:quotes,macros[`[.underline\]#xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;Ts&gt;#`].
+
*_Returns:_* `true` if `v.index()` is equal to the zero-based index of `T` in pass:quotes,macros[`[.underline\]#xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;Ts&gt;#`].
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of xref:#rvariant.recursive[`recursive_wrapper`].#
...
+
*_Returns:_* [.underline]#`xref:#rvariant.recursive.helper[_UNWRAP_RECURSIVE_](o)`, where `o` denotes a reference to the object stored in the `rvariant`#.
...
* [.candidate]#{empty}# *_Mandates:_* `I < sizeof\...(Ts)`.
+
*_Effects:_* If `v.index()` is `I`, returns [.underline]#`xref:#rvariant.recursive.helper[_UNWRAP_RECURSIVE_](o)`, where `o` denotes a reference to the object stored in the `rvariant`#. Otherwise, throws an exception of type {bad-variant-access}.
...
[.candidates]
* [.candidate]#{empty}# *_Mandates:_* The type `T` occurs exactly once in [.underline]#pass:quotes,macros[`xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;Ts&gt;`]#.
+
*_Effects:_* [.underline]#Let `VT` denote the type of the value held by `v`. If pass:quotes,macros[`xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;VT&gt;`] is the same type as `T`#, returns [.underline]#`xref:#rvariant.recursive.helper[_UNWRAP_RECURSIVE_](o)`, where `o` denotes a reference to the object stored in the `rvariant`#. Otherwise, throws an exception of type {bad-variant-access}.
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of xref:#rvariant.recursive[`recursive_wrapper`].#
...
+
*_Returns:_* A pointer to the value [.underline]#denoted by `xref:#rvariant.recursive.helper[_UNWRAP_RECURSIVE_](o)`, where `o` denotes a reference to the object stored in the `rvariant`#, if `v != nullptr` and `v\->index() == I`. Otherwise, returns `nullptr`.
...
[.candidates]
* [.candidate]#1-2)# *_Mandates:_* The type `T` occurs exactly once in [.underline]#pass:quotes,macros[`xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;Ts&gt;`]#.
+
*_Effects:_* Equivalent to: `return get_if<__i__>(v);` with _i_ being the zero-based index of `T` in [.underline]#pass:quotes,macros[`xref:#rvariant.recursive.helper[unwrap_recursive_t\]&lt;Ts&gt;`]#.
+
*_Remarks:_* [.underline]#This function is defined as deleted if `T` is a specialization of xref:#rvariant.recursive[`recursive_wrapper`].#
...
== `recursive_wrapper` helper classes
...
[.candidates]
* [.candidate]#{empty}# *_Effects:_* Denotes `*o`, if cv-unqualified non-reference type for `T` is a specialization of xref:#rvariant.recursive[`recursive_wrapper`]. Otherwise, denotes `o`.
...
+
*_Definition:_*  Let `Ts` denote an imaginary pack of types where `TT<Ts\...>` is the same type as `xref:#rvariant.xo.pack[_pack-union-t_]<TT, A, B>`. The member typedef `type` denotes `Ts\...[0]` if `sizeof\...(Ts) == 1`; otherwise, the member typedef `type` denotes `TT<Ts\...>`.
...
[WARNING]
`compact_alternative` does not unwrap xref:#rvariant.recursive[`recursive_wrapper`]. This is intentional, because doing so could lead to instantiating incomplete type on undesired timings. You may apply xref:#rvariant.recursive.helper[`unwrap_recursive_t`] manually.
...
[.candidates]
* [.candidate]#{empty}# Let `As` denote the pack of template parameters of `A` if xref:#rvariant.xo.core[`_is-specialization-of<A, TT>_`], otherwise let `As` denote a pack of single type `A`. Let `Bs` denote likewise. `_pack-union-t_` denotes `TT<Ts\...>`, where `Ts\...` is the set union of `As\...` and `Bs\...` expanded from left to right. For duplicate types, the first occurrence shall remain in `Ts\...`.
