...
+
[none]
** -- This constructor is defined as deleted unless [.underline]#`xref:#rvariant.flexibility[subset_of]<rvariant<Us\...>, rvariant>` is `true` and# `std::is_copy_constructible_v[.underline]##<U~_j_~>##` is `true` for all [.underline]#_j_, where `U~_j_~` be the _j_^th^ type in `Us`#.
...
** -- The exception specification is equivalent to the logical `AND` of `std::is_nothrow_copy_constructible_v[.underline]##<U~_j_~>##` for all [.underline]#_j_# [.underline]#If `xref:#rvariant.flexibility[equivalent_to]<rvariant<Us\...>, rvariant>` is `true`; otherwise, it is equivalent to `false`#.
