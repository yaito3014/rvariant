...
** -- [.underline]#`std::is_convertible_v<U, T>` is `true`, or the expression `T x[] = {std::forward<U>(u)}` (for an imaginary variable `x`) is valid in unevaluated context#. (_Note:_ This prevents recursive instantiation of `std::is_constructible`, even for recursive types, while preserving SFINAE-friendliness.)
--
+
