...
template<class T, class Allocator>
/* constexpr */ std::size_t hash_value(recursive_wrapper<T, Allocator> const& rw);pass:quotes[[.candidate\]#// 2#]
...
template<class... Ts>
struct hash<::temp_ns::rvariant<Ts...>>;pass:quotes[[.candidate\]#// 3#]

template<class T, class Allocator>
struct hash<::temp_ns::recursive_wrapper<T, Allocator>>;pass:quotes[[.candidate\]#// 4#]
...
[WARNING]
--
.Notes on non-"exactly-once" variant
*We strongly discourage non-"exactly-once" instantiation of variant* because the duplicate alternatives lead to *serious runtime performance issues* as the typical, well-optimized implementation of `std::hash` specialization would never consider `v.index()` during the hash generation.
--
...
* [.candidate]#2)# *_Effects:_* Equivalent to `std::hash<recursive_wrapper<T, Allocator>>{}(rw)`.
