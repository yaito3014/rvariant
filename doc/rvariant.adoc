...
** This constructor is defined as deleted unless `xref:#rvariant.helper.flexibility[subset_of]<rvariant<Ts\...>, rvariant<Us\...>>` and `std::is_copy_constructible_v<U~_j_~>` is `true` for all _j_, where `U~_j_~` be the _j_^th^ type in `Us\...`.
...
** This constructor is defined as deleted unless `xref:#rvariant.helper.flexibility[subset_of]<rvariant<Ts\...>, rvariant<Us\...>>`.
