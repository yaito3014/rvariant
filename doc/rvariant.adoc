...
// Generic constructor (non-wrapped pass:quotes[`T`])
template<class T>
constexpr rvariant(T&& t) noexcept({see-below});pass:quotes[[.candidate\]#// 4#]

// Generic constructor (pass:quotes,attributes[`{recursive_wrapper}&lt;T, A&gt;`])
template<class T>
constexpr rvariant(T&& t) noexcept({see-below}); // TODO pass:quotes[[.candidate\]#// 4#]
...
+
*_Constraints:_* In addition to the original specification,
+
[none]
** -- [.underline]#`std::remove_cvref_t<T>` is not a specialization of `{recursive_wrapper}`.#
...
+
*_Constraints:_* [.underline]#`std::is_same_v<rvariant<Us\...>, rvariant>` is `false`.#
+
...
// Generic assignment operator (non-wrapped pass:quotes[`T`])
template<class T>
constexpr rvariant& operator=(T&& t) noexcept({see-below});pass:quotes[[.candidate\]#// 3#]

// Generic assignment operator (pass:quotes,attributes[`{recursive_wrapper}&lt;T, A&gt;`])
template<class T>
constexpr rvariant& operator=(T&& t) noexcept({see-below}); // TODO pass:quotes[[.candidate\]#// 3#]
...
*_Returns:_* `*this`.
+
*_Remarks:_* This operator is defined as deleted unless pass:quotes[`std::is_copy_constructible_v[.underline\]##&lt;U~_j_~&gt;## && std::is_copy_assignable_v[.underline\]##&lt;U~_j_~&gt;##`] is `true` for all [.underline]#_j_, where `U~_j_~` be the _j_^th^ type in `Us`#.
...
* [.candidate]#2)# *_Constraints:_* For every type `U` in `Us`, there exists at least one type `T` in `Ts` such that `{unwrap_recursive_t}<T>` is the same type as `{unwrap_recursive_t}<U>`.
