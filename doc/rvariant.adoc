...
// xref:#rvariant.ctor[constructor]
AB ab{42};

{
    AB local_ab{};
} // xref:#rvariant.dtor[destructor]
...
A& a = temp_ns::xref:#rvariant.get[get]<0>(ab);
...
A* a = temp_ns::xref:#rvariant.get[get_if]<0>(&ab);
...
----
{
    // xref:#rvariant.ctor[flexible construction]
...
static_assert(temp_ns::xref:#rvariant.get[holds_alternative]<A>(ab));
...
[[rvariant.general]]
== General
* [.underline]#Underlined texts# indicate important characteristics of ``rvariant``'s extensions, in contrast to the `std::variant` counterparts.
* `_GET_` is defined as per https://eel.is/c++draft/variant.get[[variant.get\]].
...
/* extensions */
/* behaves differently mostly because of the existence of `recursive_wrapper` */

// xref:#rvariant.helper[[rvariant.helper\]], rvariant helper classes
...
// xref:#rvariant.get[[rvariant.get\]], value access
...
template<class T, class... Ts>
  constexpr T& get(rvariant<Ts...>&);
template<class T, class... Ts>
  constexpr T&& get(rvariant<Ts...>&&);
template<class T, class... Ts>
  constexpr T const& get(rvariant<Ts...> const&);
template<class T, class... Ts>
  constexpr T const&& get(rvariant<Ts...> const&&);
...
template<std::size_t I, class... Ts>
  constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>>>
    pass:quotes[[.underline\]#get#](rvariant<Ts...>*) noexcept;       // compatibility with `boost::get`
template<std::size_t I, class... Ts>
  constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>> const>
    pass:quotes[[.underline\]#get#](rvariant<Ts...> const*) noexcept; // compatibility with `boost::get`
...
// xref:#rvariant.visit[[rvariant.visit\]], visitation
template<class Visitor, class... Variants>
  constexpr pass:quotes[[.see-below\]#see below#] visit(Visitor&&, Variants&&...);
...
{
public:
  // xref:#rvariant.ctor[[rvariant.ctor\]], constructors
  constexpr rvariant::rvariant() noexcept(pass:quotes[[.see-below\]#see below#]);
  constexpr rvariant::rvariant(rvariant const&);
  constexpr rvariant::rvariant(rvariant&&) noexcept(pass:quotes[[.see-below\]#see below#]);

  template<class T>
    constexpr rvariant(T&&) noexcept(pass:quotes[[.see-below\]#see below#]);
  ...
  // xref:#rvariant.ctor[[rvariant.ctor\]], flexible constructors
  template<class... Us>
    constexpr rvariant(rvariant<Us...> const& w);
  template<class... Us>
    constexpr rvariant(rvariant<Us...>&& w) noexcept(pass:quotes[[.see-below\]#see below#]);

  // xref:#rvariant.dtor[[rvariant.dtor\]], destructor
  constexpr ~rvariant();

  // xref:#rvariant.assign[[rvariant.assign\]], assignment
  constexpr rvariant& operator=(rvariant const&);
  constexpr rvariant& operator=(rvariant&&) noexcept(pass:quotes[[.see-below\]#see below#]);

  template<class T> constexpr rvariant& operator=(T&&) noexcept(pass:quotes[[.see-below\]#see below#]);

  // xref:#rvariant.assign[[rvariant.assign\]], flexible assignment
  template<class... Us>
    constexpr rvariant& operator=(rvariant<Us...> const&);
  template<class... Us>
    constexpr rvariant& operator=(rvariant<Us...>&&) noexcept(pass:quotes[[.see-below\]#see below#]);

  // xref:#rvariant.mod[[rvariant.mod\]], modifiers
  template<class T, class... Args>
  ...
  // xref:#rvariant.status[[rvariant.status\]], value status
  constexpr bool valueless_by_exception() const noexcept;
  constexpr std::size_t index() const noexcept;

  // xref:#rvariant.subset[[rvariant.subset\]], subset
  template<class... Us>
    constexpr rvariant<Us...> subset() const& noexcept(pass:quotes[[.see-below\]#see below#]);
  template<class... Us>
    constexpr rvariant<Us...> subset() && noexcept(pass:quotes[[.see-below\]#see below#]);

  // xref:#rvariant.swap[[rvariant.swap\]], swap
  constexpr void swap(rvariant&) noexcept(pass:quotes[[.see-below\]#see below#]);
...
* [.underline]#Let `T` denote an arbitrary type. If a user provides both `T` and `xref:#rvariant.recursive[recursive_wrapper]<T>` for the template parameter of `rvariant`, the program is ill-formed.#
...
[,cpp,subs="+macros"]
----
constexpr rvariant::rvariant() noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 1#]
constexpr rvariant::rvariant(rvariant const&);pass:quotes[[.candidate\]#// 2#]
constexpr rvariant::rvariant(rvariant&&) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 3#]

template<class T>
constexpr rvariant(T&&) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 4#]
...
template<class... Us>
constexpr rvariant(rvariant<Us...>&& w) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 10#]
----
...
+
[none]
** -- This constructor is defined as deleted unless [.underline]#`xref:#rvariant.flexibility[subset_of]<rvariant<Us\...>, rvariant>` is `true` and# `std::is_copy_constructible_v<U~_j_~>` is `true` for all _j_, where `U~_j_~` be the _j_^th^ type in `Us\...`.
...
constexpr rvariant& operator=(rvariant&&) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 2#]

template<class T> constexpr rvariant& operator=(T&&) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 3#]
...
template<class... Us>
constexpr rvariant& operator=(rvariant<Us...>&& rhs) noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 5#]
----
...
+
--
*_Constraints:_* [.underline]#`xref:#rvariant.flexibility[subset_of]<rvariant<Us\...>, rvariant>` is `true`.#
...
+
--
*_Constraints:_* [.underline]#`xref:#rvariant.flexibility[subset_of]<rvariant<Us\...>, rvariant>` is `true` and# pass:quotes[`std::is_move_constructible_v[.underline\]##&lt;U~_k_~&gt;## && std::is_move_assignable_v[.underline\]##&lt;U~_k_~&gt;##`] is `true` for all [.underline]#_k_#.
...
** -- Otherwise, equivalent to pass:quotes[`emplace[.underline\]##&lt;__i__&gt;##(_GET_<_j_>(std::move(rhs)))`] [.underline]#where _i_ is the corresponding index that is determined according to the rules defined in xref:#rvariant.ctor[the flexible move constructor]#.
...
template<class... Us>
  requires (!std::same_as<rvariant<Us...>, rvariant>)
constexpr rvariant<Us...> subset() const& noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 3#]

template<class... Us>
  requires (!std::same_as<rvariant<Us...>, rvariant>)
constexpr rvariant<Us...> subset() && noexcept(pass:quotes[[.see-below\]#see below#]);pass:quotes[[.candidate\]#// 4#]
----
...
+
*_Effects:_* If `*this` holds a value, returns an `rvariant<Us\...>` object that holds the [.underline]#alternative corresponding to that of# `*this`, with its contained value direct-initialized from `_GET_&lt;__i__&gt;(*this)`, where _i_ is `this\->index()`. Otherwise, returns an `rvariant<Us\...>` object that does not hold a value.
+
*_Remarks:_*
+
[none]
** -- [.underline]#This function does not participate in overload resolution unless `xref:#rvariant.flexibility[subset_of]<rvariant<Us\...>, rvariant>` is `true`.#

** -- The exception specification is equivalent to the logical `AND` of `std::is_nothrow_copy_constructible_v<U~_j_~>` for all _j_ [.underline]#If `xref:#rvariant.flexibility[equivalent_to]<rvariant<Us\...>, rvariant>` is `true`; otherwise, it is equivalent to `false`#.

** -- [.underline]#The corresponding index on the returned `rvariant<Us\...>` object shall be determined according to the rules defined in xref:#rvariant.ctor[the flexible copy constructor].#

* [.candidate]#4)# Equivalent to the overload #3, except that:
+
[none]
** -- `_GET_&lt;__i__&gt;(*this)` is replaced with `_GET_&lt;__i__&gt;(std::move(*this))`, and
...
[,cpp,subs="+macros"]
----
variant_alternative<I, rvariant<Ts...>>::type
----
...
----
template<class T, class... Ts>
constexpr bool holds_alternative(rvariant<Ts...> const& v) noexcept;pass:quotes[[.candidate\]#// 1#]
----
...
template<std::size_t I, class... Ts>
constexpr variant_alternative_t<I, rvariant<Ts...>>&
  get(rvariant<Ts...>& v);pass:quotes[[.candidate\]#// 1#]

template<std::size_t I, class... Ts>
constexpr variant_alternative_t<I, rvariant<Ts...>>&&
  get(rvariant<Ts...>&& v);pass:quotes[[.candidate\]#// 2#]

template<std::size_t I, class... Ts>
constexpr variant_alternative_t<I, rvariant<Ts...>> const&
  get(rvariant<Ts...> const& v);pass:quotes[[.candidate\]#// 3#]

template<std::size_t I, class... Ts>
constexpr variant_alternative_t<I, rvariant<Ts...>> const&&
  get(rvariant<Ts...> const&& v);pass:quotes[[.candidate\]#// 4#]

template<class T, class... Ts> constexpr T& get(rvariant<Ts...>& v);pass:quotes[[.candidate\]#// 5#]
template<class T, class... Ts> constexpr T&& get(rvariant<Ts...>&& v);pass:quotes[[.candidate\]#// 6#]
template<class T, class... Ts> constexpr T const& get(rvariant<Ts...> const& v);pass:quotes[[.candidate\]#// 7#]
template<class T, class... Ts> constexpr T const&& get(rvariant<Ts...> const&& v);pass:quotes[[.candidate\]#// 8#]

template<std::size_t I, class... Ts>
constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>>>
  get_if(rvariant<Ts...>*) noexcept;pass:quotes[[.candidate\]#// 9#]

template<std::size_t I, class... Ts>
constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>> const>
  get_if(rvariant<Ts...> const* v) noexcept;pass:quotes[[.candidate\]#// 10#]

template<class T, class... Ts>
constexpr std::add_pointer_t<T>
  get_if(rvariant<Ts...>* v) noexcept;pass:quotes[[.candidate\]#// 11#]

template<class T, class... Ts>
constexpr std::add_pointer_t<T const>
  get_if(rvariant<Ts...> const* v) noexcept;pass:quotes[[.candidate\]#// 12#]


template<std::size_t I, class... Ts>
constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>>>
  pass:quotes[[.underline\]#get#](rvariant<Ts...>*) noexcept;         // compatibility with `boost::get`pass:quotes[[.candidate\]#// 13#]

template<std::size_t I, class... Ts>
constexpr std::add_pointer_t<variant_alternative_t<I, rvariant<Ts...>> const>
  pass:quotes[[.underline\]#get#](rvariant<Ts...> const* v) noexcept; // compatibility with `boost::get`pass:quotes[[.candidate\]#// 14#]

template<class T, class... Ts>
constexpr std::add_pointer_t<T>
  pass:quotes[[.underline\]#get#](rvariant<Ts...>* v) noexcept;       // compatibility with `boost::get`pass:quotes[[.candidate\]#// 15#]

template<class T, class... Ts>
constexpr std::add_pointer_t<T const>
  pass:quotes[[.underline\]#get#](rvariant<Ts...> const* v) noexcept; // compatibility with `boost::get`pass:quotes[[.candidate\]#// 16#]
----
...
* [.candidate]#11-12)# TODO

* [.candidate]#13-14)# Provided for compatibility with `boost::get`.
+
*_Effects:_* Equivalent to `get_if<I>(v)`.

* [.candidate]#15-16)# Provided for compatibility with `boost::get`.
+
*_Effects:_* Equivalent to `get_if<T>(v)`.
...
----
template<class Visitor, class... Variants>
constexpr pass:quotes[[.see-below\]#see below#] visit(Visitor&& vis, Variants&&... vars);pass:quotes[[.candidate\]#// 1#]

template<class R, class Visitor, class... Variants>
constexpr R visit(Visitor&& vis, Variants&&... vars);pass:quotes[[.candidate\]#// 2#]

template<class Self, class Visitor>
constexpr decltype(auto) visit(this Self&&, Visitor&&);pass:quotes[[.candidate\]#// 3#]

template<class R, class Self, class Visitor>
constexpr R visit(this Self&&, Visitor&&);pass:quotes[[.candidate\]#// 4#]
----
...
+
_Formally:_  Let `Ts` denote an imaginary pack of types where `TT<Ts\...>` is the same type as `xref:#rvariant.xo.pack[_pack-union-t_]<TT, A, B>`. `compact_alternative` defines a member typedef `type` that names the first type in `Ts\...` if `sizeof\...(Ts) == 1`; otherwise, it names `TT<Ts\...>`.
--
...
----
template<template<class...> class TT, class A, class B>
using pass:quotes[_pack-union-t_] = pass:quotes[[.see-below\]#see below#]; // exposition-onlypass:quotes[[.candidate\]#// 1#]
----
