...
https://eel.is/c+\+draft/variant.syn[/* all features commented below defined as per standard */]
...
[.candidates]
--
* [.candidate]#1)# Let `As` denote the pack of template parameters of `A` if xref:#rvariant.xo.core[`_is-specialization-of<A, TT>_`], otherwise let `As` denote a pack of single type `A`. Let `Bs` denote likewise. `pack_union` defines the member type `::type` as `TT<Ts\...>`, where `Ts\...` is the union of `As\...` and `Bs\...`. The order of types shall be preserved.
...
>
struct pass:quotes[_subset_like_]<TT<Ts...>, UU<Us...>> : std::true_type {}; // exposition-onlypass:quotes[[.candidate\]#// 1#]
