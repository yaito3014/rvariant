...
public:
    ...
    constexpr explicit scoped_allocation(Alloc const& a) : alloc_(a), ptr_(std::allocator_traits<Alloc>::allocate(alloc_, 1)) {}

    template<class... Args>
    constexpr explicit scoped_allocation(Alloc const& a, std::in_place_t, Args&&... args) : scoped_allocation(a) {
    ...
    constexpr pointer get() const noexcept { return std::to_address(ptr_); }
...
public:
    ...
    constexpr explicit indirect(std::allocator_arg_t, Allocator const& a) : alloc_(a), ptr_(make_obj()) {}

    constexpr indirect(indirect const& other)
        : indirect(std::allocator_arg, std::allocator_traits<Allocator>::select_on_container_copy_construction(other.alloc_), other) {}

    constexpr indirect(std::allocator_arg_t, Allocator const& a, indirect const& other)
        : alloc_(a), ptr_(other.ptr_ ? make_obj(std::as_const(*other.ptr_)) : nullptr) {
        static_assert(std::is_copy_constructible_v<T>);
    }

    constexpr indirect(indirect&& other) noexcept : alloc_(std::move(other.alloc_)), ptr_(std::exchange(other.ptr_, nullptr)) {}

    constexpr indirect(std::allocator_arg_t, Allocator const& a, indirect&& other) noexcept(std::allocator_traits<Allocator>::is_always_equal::value)
        : alloc_(a), ptr_(alloc_ == other.alloc_ ? std::exchange(other.ptr_, nullptr) : make_obj(std::as_const(*other.ptr_))) {}
    ...
    constexpr explicit indirect(U&& u) : ptr_(make_obj(std::forward<U>(u))) {}
    ...
    constexpr explicit indirect(std::allocator_arg_t, Allocator const& a, U&& u) : alloc_(a), ptr_(make_obj(std::forward<U>(u))) {}
    ...
    constexpr explicit indirect(std::in_place_t, Us&&... us) : ptr_(make_obj(std::forward<Us>(us)...)) {}

    template<class... Us>
        requires requires { requires std::is_constructible_v<T, Us...>; }
    constexpr explicit indirect(std::allocator_arg_t, Allocator const& a, std::in_place_t, Us&&... us) : alloc_(a), ptr_(make_obj(std::forward<Us>(us)...)) {}
    ...
    constexpr explicit indirect(std::in_place_t, std::initializer_list<I> il, Us&&... us) : ptr_(make_obj(il, std::forward<Us>(us)...)) {}

    template<class I, class... Us>
        requires requires { requires std::is_constructible_v<T, std::initializer_list<I>&, Us...>; }
    constexpr explicit indirect(std::allocator_arg_t, Allocator const& a, std::in_place_t, std::initializer_list<I> il, Us&&... us)
        : alloc_(a), ptr_(make_obj(il, std::forward<Us>(us)...)) {}
    ...
    constexpr indirect& operator=(indirect&& other) noexcept(std::allocator_traits<Allocator>::propagate_on_container_move_assignment::value ||
    ...
    [[nodiscard]] constexpr T&& operator*() && noexcept { return std::move(*ptr_); }
    [[nodiscard]] constexpr T const&& operator*() const&& noexcept { return std::move(*ptr_); }
    ...
    friend constexpr void swap(indirect& lhs, indirect& rhs) noexcept(noexcept(lhs.swap(rhs))) { return lhs.swap(rhs); }

    template<class U, class AA>
    friend constexpr bool operator==(indirect const& lhs, indirect<U, AA> const& rhs) noexcept(noexcept(*lhs == *rhs)) {
        if (lhs.valueless_after_move() || rhs.valueless_after_move()) return lhs.valueless_after_move() == rhs.valueless_after_move();
        return *lhs == *rhs;
    }
    ...
    template<class U>
    friend constexpr bool operator==(indirect const& lhs, U const& rhs) noexcept(noexcept(*lhs == rhs)) {
...
private:
    ...
    template<class... Args>
    constexpr T* make_obj(Args&&... args) const {
...
indirect(std::allocator_arg_t, Allocator, Value) -> indirect<Value, typename std::allocator_traits<Allocator>::template rebind_alloc<Value>>;
...
struct hash<yk::indirect<T, A>> {
    std::size_t operator()(yk::indirect<T, A> const& i) const {
        if (i.valueless_after_move()) return 33 - 4zu;  // arbitary number
        return hash<T>{}(*i);
    }
};
